...
\begin{description}
  ...
  \item[Como llaves maestra:] una llave maestra simétrica se usa para derivar
    otras llaves simétricas (e. g. llaves de cifrado, o de envoltura). También
  ...
  \item[Para el transporte de llaves:] llaves utilizadas en esquemas
    ...
    llaves. Se usan para proteger otras llaves (e. g. de cifrado, de
    envoltura) o información relacionada (e. g.
...
\begin{itemize}
  ...
  \item Limita el total de daños si una sola llave se ve comprometida.

  \item Limita el uso de un algoritmo en particular (\textit{particular}
    en el sentido de la instancia misma del algoritmo).

  \item Limita el tiempo disponible para intentos de ataques sobre los
    mecanismos que protegen a la llave del acceso sin autorización.

  \item Limita el periodo en el cual la información puede verse comprometida
    por revelaciones inadvertidas de llaves a entidades sin autorización.

  \item Limita el tiempo disponible para ataque criptoanalíticos.

\end{itemize}

Al momento de establecer un criptoperiodo se deben sopsar cuidadosamente los
riesgos y consecuencias de una llave comprometida. Entre los factores que
se deben tomar en cuenta, están:
...
\begin{itemize}
  ...
  \item El entorno de operacion (e. g. un lugar de acceso restringido, una
    terminal pública).
...
\begin{description}
  ...
  \item[Garantía de validez de parámetros de dominio:] los parámetros de
    dominio son usado por los algoritmos de llave pública al momento de la
