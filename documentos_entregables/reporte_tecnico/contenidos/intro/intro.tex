...
debe de cumplir un sistema tokenizado, sim embargo, no se explica cómo debe de
...
Bluepay TokenShield:
En este servicio, a pesar de que se ofrecen dos formas distintas de tokenizar,
nunca se aclara cómo es que funciona alguno de estos procesos. Además en una
breve descripción del flujo de datos del servicio, se dice que con el token
se recupera y descifra la información bancaria sensible, lo que genera
confusión, pues no se sabe si la tokenización es hace por medio de de cifrados
que preservan el formato, o por medio de tablas que contienen un pares
PAN-token.
...
tokenización como el programa de generación de \glspl{gl:token}.
