/**
 * \file
 * \brief Implementación de pruebas del lector de aleatoriedad desde hardware.
 *
 * Proyecto Lovelace.
 */

#include "cabeceras/aleatoriedad_hardware.hh"
#include "cabeceras/aleatoriedad_hardware_prueba.hh"
#include <iostream>

using namespace Implementaciones;
using namespace ImplementacionesPruebas;
using namespace std;

AleatoriedadHardwarePrueba::AleatoriedadHardwarePrueba()
: Utilidades::Prueba{"pruebas de aleatoriedad desde hardware"}
{
  mListaDePruebas.push_back(Utilidades::FuncionDePrueba{
    "operación desde hardware",
    AleatoriedadHardwarePrueba::probarOperacion
  });
}

/**
 * Prueba la llamada a la instrucción RDSEED de intel.
 *
 * \return Estado de la prueba.
 */

bool AleatoriedadHardwarePrueba::probarOperacion()
{
  try
  {
    AleatoriedadHardware temporal{};
  }
  catch(CryptoPP::RDSEED_Err &error)
  {
    cout << "Sin entropía por hardware." << endl;
    return true;
  }

  AleatoriedadHardware aleatoriedad {};
  cout << "Alatorio uno (8): ";
  Arreglo<unsigned char> resultadoUno = aleatoriedad.operar({8});
  for (unsigned int i = 0; i < resultadoUno.obtenerNumeroDeElementos(); i++)
    cout << "0x" << hex << (0xFF & resultadoUno[i]) << " ";
  cout << endl;

  Arreglo<unsigned char> resultadoDos = aleatoriedad.operar({16});
  cout << "Alatorio dos (16): ";
  for (unsigned int i = 0; i < resultadoDos.obtenerNumeroDeElementos(); i++)
    cout << "0x" << hex << (0xFF & resultadoDos[i]) << " ";
  cout << endl;

  Arreglo<unsigned char> resultadoTres = aleatoriedad.operar({128});
  cout << "Alatorio tres (128): ";
  for (unsigned int i = 0; i < resultadoTres.obtenerNumeroDeElementos(); i++)
    cout << "0x" << hex << (0xFF & resultadoTres[i]) << " ";
  cout << endl;

  return true;
}
